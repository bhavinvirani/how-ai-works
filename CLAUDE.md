...
pnpm icons [--preview]        # regenerate favicon/OG/manifest/robots into public/
```
...
- `src/seo/site.ts` — site identity (name, origin, author, social card, verification tokens, analytics endpoint). One source for the `<head>`, the manifest, `robots.txt`, and the OG image, none of which can import each other.
- `src/components/seo/SeoHead.astro` — every machine-facing tag. Pages opt into `article`, `noindex`, dates and JSON-LD through `BaseLayout`'s `seo` prop; the testable rules live in `src/lib/seo/`.
- `scripts/brand.mjs` + `make-icons.mjs` — the icon and social-card *design*, and the Playwright rasteriser that emits them. **Everything in `public/` is generated** — never hand-edit it. See `docs/SEO.md`.
...
11. **Discovery is generated, and honest.** Indexable: home, `/map`, the 60 lessons. `noindex`: `/gallery`, `/search`, `/progress`, `/404` — declared on the page AND in `scripts/sitemap.mjs`, and the two must agree. PR previews are `noindex, nofollow` site-wide, because they share an origin with the live site. Everything in `public/` is generated by `pnpm icons` — never hand-edit it, and re-run it when you add a unit, because each lesson has its own social card. The site counts visits (GoatCounter, cookieless), which is why `/progress` and the README say so; if that ever changes, change them back. `docs/SEO.md` has the rest, including why `robots.txt` does nothing here.
