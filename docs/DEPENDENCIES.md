...
| Package                                        | For                                                   | Notes                                                                                            |
| ---------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `nanostores`, `@nanostores/persistent`         | Sharing state between islands, and the progress store | ~1 KB. Arrives with the ProgressStore.                                                           |
...
| `@xyflow/react`                                | The concept map on `/map`                             | One page only, lazily loaded, with its own 160 KB budget.                                        |
| `pagefind`                                     | Static search                                         | Runs over `dist/` after the build; ships its own client bundle, so it gets its own budget entry. |
