...
## Social preview images — **built**

Kept here, struck through, because the trigger condition on this page is the
useful part and this one turned out to be wrong.

**It said:** wait until links are shared and land flat. **What happened:** the
whole discovery pass landed at once, and a card per lesson was a few hours on
top of a card per site rather than a separate project — the input (`title`,
`part`) was already in frontmatter, and the renderer was already written for the
favicon. Deferring would have cost more than doing it.

What shipped: one shared card, plus sixty per-lesson cards generated by
`pnpm icons` and committed. The trap this entry warned about — `og:image` is
fetched by _someone else's_ server, so a base-relative path silently produces a
card that looks identical to no card at all — is handled and asserted in
`tests/e2e/seo.spec.ts`. See `docs/SEO.md`.

**The lesson for the rest of this page:** a trigger of the form "wait until
somebody wants it" is right for work with an ongoing cost (a translation, a
deployed service, a second language). It is wrong for work that is nearly free
once adjacent work is being done anyway. Check which kind an entry is before
trusting its trigger.
